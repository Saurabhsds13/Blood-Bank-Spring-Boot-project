...
public class DonorController {
	...
	public ResponseEntity<Donor> updateDonor(@Validated @RequestBody Donor donor){
		
		Donor donorOld = donordao.findOneByEmail(donor.getEmailID());
		
		if(donorOld == null) {
			return ResponseEntity.notFound().build();
			}

		donor.setDonorId(donorOld.getDonorId());
		donordao.save(donor);
